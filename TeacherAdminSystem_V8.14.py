...
class TeacherStatsApp:
    ...
    def get_xl_safe(self, path, password):
        """讀取可能加密的 Excel 檔案"""
        if not password:
            try:
                ...
                if "encrypted" in str(e).lower() or "password" in str(e).lower():
                    messagebox.showwarning("提示", "此檔案已加密，請輸入密碼後再讀取。")
                else:
                    messagebox.showerror("讀取失敗", str(e))
                return None
        
        if msofficecrypt is None:
            messagebox.showerror("缺少組件", "讀取加密檔案需要 msoffice-crypt-python 套件。\n請在終端機執行: pip install msoffice-crypt-python")
            return None
        ...
        try:
            ...
            messagebox.showerror("解密出錯", f"密碼錯誤或檔案格式不支援：\n{str(e)}")
            return None
    ...
    def setup_tab1(self, f):
        ...
        ttk.Label(file_f, text="密碼:").pack(side=tk.LEFT, padx=5)
        self.pw1_entry = ttk.Entry(file_f, show="*", width=15)
        self.pw1_entry.pack(side=tk.LEFT, padx=5)
        
        ttk.Label(file_f, text="教師工作表名稱:").pack(side=tk.LEFT, padx=5)
        self.teacher_sheet_entry = ttk.Entry(file_f, width=12)
        self.teacher_sheet_entry.insert(0, "教師")
        self.teacher_sheet_entry.pack(side=tk.LEFT, padx=5)

        ttk.Button(file_f, text="重整", command=self.refresh_tab1_titles).pack(side=tk.LEFT)
    ...
    def refresh_tab1_titles(self):
        ...
        if not sel_cats:
            ...
            return
        try:
            xl = self.get_xl_safe(self.file1_path, self.pw1_entry.get())
            if xl is None: return
            all_dfs = []
            for cat in sel_cats:
                # 如果是 "教師" 類別，則使用自定義工作表名稱
                sheet_name = self.teacher_sheet_entry.get() if cat == "教師" else cat
                if sheet_name in xl.sheet_names:
                    all_dfs.append(xl.parse(sheet_name))
            if not all_dfs:
                return
    ...
    def setup_tab2(self, f):
        ...
        pw_f = ttk.Frame(file_f)
        pw_f.pack(fill=tk.X, pady=5)
        ttk.Label(pw_f, text="密碼:").pack(side=tk.LEFT, padx=5)
        self.pw2_entry = ttk.Entry(pw_f, show="*", width=15)
        self.pw2_entry.pack(side=tk.LEFT, padx=5)
        ttk.Button(pw_f, text="重整", command=self.reload_file2_with_pw).pack(side=tk.LEFT)
    ...
    def load_file2_v7(self):
        ...
        self.reload_file2_with_pw()

    def reload_file2_with_pw(self):
        if not self.file2_path: return
        xl = self.get_xl_safe(self.file2_path, self.pw2_entry.get())
        if xl is None: return
    ...
    def gen_vice_president_report(self):
        ...
        try:
            ...
            return
        custom_teacher_sheet = self.teacher_sheet_entry.get()
        target_sheets = [custom_teacher_sheet if s == "教師" else s for s in ["教師", "約聘教師", "專案教學人員", "講座教授", "客座人員", "研究人員", "稀少性科技人員", "醫事人員", "新制職員", "駐衛警察隊", "技工工友合計"]]
        try:
            xl = self.get_xl_safe(self.file1_path, self.pw1_entry.get())
            if xl is None: return
    ...
    def gen_d5_logic(self):
        ...
        try:
            ...
            # 教師工作表
            teacher_sheet_name = self.teacher_sheet_entry.get()
            if teacher_sheet_name in all_sheets:
                df = xl.parse(teacher_sheet_name)
                # 排除職稱為空白的資料
                c_t = next((c for c in df.columns if any(k in str(c) for k in ["職稱", "職別"])), None)
                if c_t:
                    df = df[df[c_t].astype(str).str.strip().replace(['nan','','None','nan '], pd.NA).notna()]
                ...
                if not df_normal.empty:
                    df_normal['D5類別'] = "教師"
                    df_normal['來源工作表'] = teacher_sheet_name
                    cat_data["教師"].append(df_normal)
                
                if not df_tech.empty:
                    df_tech['D5類別'] = "聘任"
                    df_tech['來源工作表'] = f"{teacher_sheet_name}(專業技術人員)"
    ...
    def gen_unpaid_leave_logic(self):
        ...
        target_sheets = ["新制職員", "助教", self.teacher_sheet_entry.get(), "稀少性科技人員", "研究人員"]
        try:
            xl = self.get_xl_safe(self.file1_path, self.pw1_entry.get())
            if xl is None: return
            found_dfs = []
    ...
    def get_app_core_data(self):
        ...
        try:
            xl = self.get_xl_safe(self.file1_path, self.pw1_entry.get())
            if xl is None: return None, None, None
            all_dfs = []
            for c in sel_cats:
                # 如果類別是 "教師"，則使用自定義工作表名稱
                sheet_name = self.teacher_sheet_entry.get() if c == "教師" else c
                if sheet_name in xl.sheet_names: 
                    all_dfs.append(xl.parse(sheet_name))
