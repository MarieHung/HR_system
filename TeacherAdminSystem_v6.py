...
class TeacherStatsApp:
    def __init__(self, root):
        self.root = root
        self.root.title("教師資料管理中心 - 行政綜合業務系統 (V6)")
    ...
    def setup_tab7(self, f):
        main = ttk.Frame(f, padding="20")
        main.pack(fill=tk.BOTH, expand=True)
        ttk.Label(main, text="個人資料使用申請 - 工作表智慧切換與 Excel 匯出", font=('Microsoft JhengHei', 14, 'bold')).pack(pady=5)
    ...
    def load_file1(self):
        ...
        try:
            ...
            possible_cat_cols = [c for c in self.df1.columns if any(k in str(c) for k in ['類別', '職別', '身分', '屬性'])]
    ...
    def get_combined_app_data(self):
        ...
        try:
            ...
            if not all_dfs:
                messagebox.showerror("錯誤", "找不到對應工作表")
                return None, None, None
            ...
            messagebox.showerror("資料讀取失敗", str(e))
            return None, None, None
