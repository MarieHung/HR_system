...
class TeacherStatsApp:
    ...
    def setup_tab9(self, f):
        ...
        info_f = ttk.LabelFrame(main, text="設定與說明", padding=10)
        info_f.pack(fill=tk.X, pady=10)
        ttk.Label(info_f, text="說明：本功能從『各系所教師』檔案中統計指定校區的人數。", foreground="blue").pack(anchor=tk.W)
        
        # 選擇校區
        select_f = ttk.Frame(info_f)
        select_f.pack(fill=tk.X, pady=10)
        ttk.Label(select_f, text="請選擇校區:").pack(side=tk.LEFT, padx=5)
        self.campus_var = tk.StringVar(value="林口校區")
        campus_cb = ttk.Combobox(select_f, textvariable=self.campus_var, values=["林口校區", "校本部", "公館校區"], state="readonly")
        campus_cb.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(main, text="📊 開始統計校區人數", command=self.calculate_campus_stats).pack(pady=5)
        
        self.campus_out = tk.Text(main, font=('Microsoft JhengHei', 11), bg="#f0f8ff")
        self.campus_out.pack(fill=tk.BOTH, expand=True, pady=10)
    ...
    def calculate_campus_stats(self):
        ...
        target_campus = self.campus_var.get()
        # 定義要統計的工作表
        target_sheets = ["新制職員", "助教", "教師", "約聘教師", "專案教學人員", "講座教授", "客座教授", "醫事人員", "稀少性科技人員", "技工工友合計"]
        
        try:
            xl = pd.ExcelFile(self.file1_path)
            self.campus_out.delete(1.0, tk.END)
            self.campus_out.insert(tk.END, f"【{target_campus} 人數統計結果】\n")
            ...
            for sheet in target_sheets:
                if sheet in xl.sheet_names:
                    ...
                    if campus_col:
                        ...
                        count = mask.sum()
                        if count > 0:
                            details.append(f"● {sheet:<15} | {count:>5} 名")
                            total_count += count
                        else:
                            # 如果指定欄位沒找到，嘗試全表搜尋
                            ...
                            if count_any > 0:
                                details.append(f"● {sheet:<15} | {count_any:>5} 名 (跨欄位匹配)")
                                total_count += count_any
                    else:
                        # 全表搜尋
                        mask_any = df.astype(str).apply(lambda x: x.str.contains(target_campus)).any(axis=1)
                        count_any = mask_any.sum()
                        if count_any > 0:
                            details.append(f"● {sheet:<15} | {count_any:>5} 名 (全表搜尋)")
                            total_count += count_any
            
            if not details:
                self.campus_out.insert(tk.END, f"未在各工作表中找到屬於『{target_campus}』的人員紀錄。\n")
            else:
                for line in details:
                    self.campus_out.insert(tk.END, line + "\n")
                self.campus_out.insert(tk.END, "-"*50 + f"\n總計人數：{total_count} 名\n" + "="*50)
