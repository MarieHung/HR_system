...
class TeacherStatsApp:
    def __init__(self, root):
        ...
        # 建立 10 個主分頁
        tabs = [" 🔍 職稱統計 ", " 📅 到離查詢 ", " 📊 給副校長 ", " 📄 D5 報表 ", " 🛌 留職停薪 ", " 🌏 赴大陸統計 ", " 📝 個資申請單 ", " 📑 人事異動單 ", " 🏫 校區人數 ", " 🎂 年齡統計 "]
        self.tab_frames = []
        for text in tabs:
  ...
  # --- 分頁 7: 個資申請單 ---
    ...
    def refresh_tab9_sheets(self):
        if not self.file1_path:
            return
        if not hasattr(self, 'sheet_container9'):
            return
        try:
            xl = pd.ExcelFile(self.file1_path)
    ...
    def setup_tab10(self, f):
        ...
        main = ttk.Frame(f, padding="20")
        main.pack(fill=tk.BOTH, expand=True)
        ttk.Label(main, text="年齡區間人數統計", font=('Microsoft JhengHei', 14, 'bold')).pack(pady=5)
        ...
        cat_f = ttk.LabelFrame(main, text="1. 請勾選人員類別 (對應工作表)", padding=10)
        cat_f.pack(fill=tk.X, pady=5)
        cat_grid = ttk.Frame(cat_f)
        ...
        categories = ["新制職員", "助教", "技工工友合計", "教師", "約聘教師", "駐衛警察隊", "專案教學人員", "講座教授", "客座教授", "醫事人員", "稀少性科技人員"]
        for i, cat in enumerate(categories):
            var = tk.BooleanVar(value=(cat in ["教師", "約聘教師", "專案教學人員"]))
            self.tab10_cat_vars[cat] = var
            ttk.Checkbutton(cat_grid, text=cat, variable=var).grid(row=i//6, column=i%6, sticky=tk.W, padx=10, pady=2)

        btn_f = ttk.Frame(main)
        btn_f.pack(fill=tk.X, pady=5)
        ttk.Button(btn_f, text="📊 開始統計年齡區間", command=self.calculate_age_stats).pack(side=tk.LEFT, padx=5)

        table_f = ttk.Frame(main)
        table_f.pack(fill=tk.BOTH, expand=True, pady=5)
        cols = ("一級單位", "職稱", "<55歲", ">=55且<60歲", ">=60歲", "總計")
        self.age_tree = ttk.Treeview(table_f, columns=cols, show="headings")
        vsb = ttk.Scrollbar(table_f, orient="vertical", command=self.age_tree.yview)
        hsb = ttk.Scrollbar(table_f, orient="horizontal", command=self.age_tree.xview)
        self.age_tree.configure(yscrollcommand=vsb.set, xscrollcommand=hsb.set)

        widths = {"一級單位": 180, "職稱": 140, "<55歲": 90, ">=55且<60歲": 120, ">=60歲": 90, "總計": 80}
        for col in cols:
            self.age_tree.heading(col, text=col)
            self.age_tree.column(col, width=widths.get(col, 100), anchor=tk.CENTER)
        self.age_tree.grid(row=0, column=0, sticky="nsew")
        vsb.grid(row=0, column=1, sticky="ns")
        hsb.grid(row=1, column=0, sticky="ew")
        table_f.grid_rowconfigure(0, weight=1)
        table_f.grid_columnconfigure(0, weight=1)

        self.age_status = ttk.Label(main, text="", foreground="gray")
        self.age_status.pack(anchor=tk.W)
    ...
    def calculate_age_stats(self):
        ...
        selected_categories = [cat for cat, var in self.tab10_cat_vars.items() if var.get()]
        if not selected_categories:
            messagebox.showwarning("提示", "請至少勾選一個人員類別")
            return
        ...
        try:
            ...
            frames = []
            missing_sheets = []
            for sheet in selected_categories:
                if sheet not in xl.sheet_names:
                    missing_sheets.append(sheet)
                    continue
                df = pd.read_excel(TEACHER_DATA_PATH, sheet_name=sheet)
                if not df.empty:
                    df["人員類別"] = sheet
                    frames.append(df)

            if not frames:
                messagebox.showwarning("提示", "勾選的人員類別沒有可統計資料")
                return

            df_all = pd.concat(frames, ignore_index=True)
            unit_col = self.find_matching_column(df_all, ["一級單位"])
            title_col = self.find_matching_column(df_all, ["職稱", "職別"])
            age_col = self.find_matching_column(df_all, ["年齡"])

            missing_cols = []
            if not unit_col:
                missing_cols.append("一級單位")
            if not title_col:
                missing_cols.append("職稱")
            if not age_col:
                missing_cols.append("年齡")
            if missing_cols:
                messagebox.showerror("欄位缺失", "找不到必要欄位：" + "、".join(missing_cols))
                return

            work = df_all[[unit_col, title_col, age_col]].copy()
            work.columns = ["一級單位", "職稱", "年齡"]
            work["一級單位"] = work["一級單位"].astype(str).str.strip()
            work["職稱"] = work["職稱"].astype(str).str.strip()
            work["年齡"] = pd.to_numeric(work["年齡"], errors="coerce")
            work = work[
                work["一級單位"].replace(["nan", "None", ""], pd.NA).notna()
                & work["職稱"].replace(["nan", "None", ""], pd.NA).notna()
                & work["年齡"].notna()
            ]

            if work.empty:
                messagebox.showwarning("提示", "沒有可統計的年齡資料")
                return

            work["年齡區間"] = pd.cut(
                work["年齡"],
                bins=[-float("inf"), 55, 60, float("inf")],
                right=False,
                labels=["<55歲", ">=55且<60歲", ">=60歲"],
            )

            pivot = (
                work.groupby(["一級單位", "職稱", "年齡區間"], observed=False)
                .size()
                .unstack(fill_value=0)
                .reset_index()
            )
            for col in ["<55歲", ">=55且<60歲", ">=60歲"]:
                if col not in pivot.columns:
                    pivot[col] = 0
            pivot["總計"] = pivot[["<55歲", ">=55且<60歲", ">=60歲"]].sum(axis=1)
            pivot = pivot.sort_values(["一級單位", "職稱"])
            ...
            for _, row in pivot.iterrows():
                ...
                self.age_tree.insert("", tk.END, values=values)

            status = f"完成：{len(work)} 筆有效年齡資料，{len(pivot)} 組一級單位/職稱"
            if missing_sheets:
                status += "；找不到工作表：" + "、".join(missing_sheets)
            self.age_status.config(text=status)

        except Exception as e:
            messagebox.showerror("統計失敗", str(e))

    def find_matching_column(self, df, keywords):
        return next((col for col in df.columns if any(keyword in str(col) for keyword in keywords)), None)
