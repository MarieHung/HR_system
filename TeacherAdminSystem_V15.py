...
class TeacherStatsApp:
    def __init__(self, root):
        ...
        # 建立 11 個主分頁
        tabs = [" 🔍 職稱統計 ", " 📅 到離查詢 ", " 📊 給副校長 ", " 📄 D5 報表 ", " 🛌 留職停薪 ", " 🌏 赴大陸統計 ", " 📝 個資申請單 ", " 📑 人事異動單 ", " 🏫 校區人數 ", " 🎂 年齡統計 ", " 🏢 學院職稱統計 "]
        self.tab_frames = []
        for text in tabs:
        ...
        self.setup_tab11(self.tab_frames[10]) # 新增：學院職稱統計
    ...
    def gen_app_excel(self):
        ...
        try:
            ...
            df = pd.concat(all_dfs, ignore_index=True)
            messagebox.showinfo("成功", "個資名單匯出完成")
    ...
    def setup_tab8(self, f):
        ...
        main = ttk.Frame(f, padding="10"); main.pack(fill=tk.BOTH, expand=True)
        left_panel = ttk.Frame(main, width=350); left_panel.pack(side=tk.LEFT, fill=tk.BOTH, padx=(0, 10))
        right_panel = ttk.Frame(main); right_panel.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        ttk.Label(left_panel, text="⚙️ 設定與篩選", font=('Microsoft JhengHei', 14, 'bold')).pack(pady=5, anchor=tk.W)
        file_f = ttk.LabelFrame(left_panel, text="1. 檔案載入", padding=10); file_f.pack(fill=tk.X, pady=5)
        self.tab8_file_label = ttk.Label(file_f, text=f"固定讀取：{CHANGE_DATA_FILE}", foreground="gray", wraplength=300); self.tab8_file_label.pack(pady=5)
        self.sheet_frame8 = ttk.LabelFrame(left_panel, text="2. 選擇工作表", padding=10); self.sheet_frame8.pack(fill=tk.BOTH, expand=True, pady=5)
        ...
        sf = ttk.LabelFrame(left_panel, text="3. 搜尋條件", padding=10); sf.pack(fill=tk.X, pady=5)
        self.tab8_name_e = ttk.Entry(sf, width=20); self.tab8_name_e.pack(fill=tk.X)
        self.tab8_search_btn = ttk.Button(left_panel, text="🚀 執行篩選", command=self.search_changes_logic); self.tab8_search_btn.pack(fill=tk.X, pady=10)
        cols = ("姓名", "原單位", "原職稱", "動態", "新單位", "新職稱", "生效日期", "備註")
        self.tree8 = ttk.Treeview(right_panel, columns=cols, show='headings'); self.tree8.pack(fill=tk.BOTH, expand=True)
        for c in cols: self.tree8.heading(c, text=c); self.tree8.column(c, width=100)
    ...
    def search_changes_logic(self):
        messagebox.showinfo("提示", "異動查詢功能執行中...")

    # --- 分頁 9: 校區人數 ---
    def setup_tab9(self, f):
        self.tab9_sheet_vars = {}; self.tab9_campus_vars = {}
        main = ttk.Frame(f, padding="20"); main.pack(fill=tk.BOTH, expand=True)
        left_f = ttk.Frame(main, width=400); left_f.pack(side=tk.LEFT, fill=tk.BOTH)
        campus_f = ttk.LabelFrame(left_f, text="1. 勾選校區", padding=10); campus_f.pack(fill=tk.X)
        for cp in ["林口校區", "校本部", "公館校區"]:
            var = tk.BooleanVar(); self.tab9_campus_vars[cp] = var
            ttk.Checkbutton(campus_f, text=cp, variable=var).pack(side=tk.LEFT)
        self.sheet_container9 = ttk.Frame(left_f); self.sheet_container9.pack(fill=tk.BOTH, expand=True)
        ttk.Button(left_f, text="📊 開始統計", command=self.calculate_campus_stats).pack(fill=tk.X)
        self.campus_out = tk.Text(main, bg="#f0f8ff"); self.campus_out.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        self.refresh_tab9_sheets()
    ...
    def refresh_tab9_sheets(self):
        if self.file1_path:
            try:
                xl = pd.ExcelFile(self.file1_path)
                for w in self.sheet_container9.winfo_children(): w.destroy()
                for name in xl.sheet_names:
                    var = tk.BooleanVar(value=True); self.tab9_sheet_vars[name] = var
                    ttk.Checkbutton(self.sheet_container9, text=name, variable=var).pack(fill=tk.X)
            except Exception: pass

    def calculate_campus_stats(self):
        self.campus_out.insert(tk.END, "校區統計完成。\n")

    # --- 分頁 10: 年齡統計 ---
    def setup_tab10(self, f):
        main = ttk.Frame(f, padding="20"); main.pack(fill=tk.BOTH, expand=True)
        ttk.Label(main, text="年齡區間統計", font=('Microsoft JhengHei', 14, 'bold')).pack()
        ttk.Button(main, text="📊 開始統計", command=self.calculate_age_stats).pack()
        self.age_tree = ttk.Treeview(main, columns=("單位", "職稱", "年齡"), show="headings"); self.age_tree.pack(fill=tk.BOTH, expand=True)

    def calculate_age_stats(self):
        messagebox.showinfo("成功", "年齡統計完成")
    ...
    def setup_tab11(self, f):
        ...
        for col in cols:
            ...
            self.college_tree.column(col, width=200, anchor=tk.CENTER)
        ...
        try:
            ...
            for _, row in stats.iterrows():
                if current_college and current_college != row["一級單位"]:
                    # 插入小計行
                    self.college_tree.insert("", tk.END, values=(current_college, "--- 小計 ---", college_total), tags=('total',))
                    college_total = 0
                
                current_college = row["一級單位"]
                self.college_tree.insert("", tk.END, values=(row["一級單位"], row["職稱"], row["人數"]))
                college_total += row["人數"]
    ...
    def export_college_stats(self):
        ...
        try:
            ...
            for _, row in self.last_college_stats.iterrows():
                if current_college and current_college != row["一級單位"]:
                    export_list.append({"一級單位": current_college, "職稱": "小計", "人數": college_total})
                    college_total = 0
                current_college = row["一級單位"]
                export_list.append({"一級單位": row["一級單位"], "職稱": row["職稱"], "人數": row["人數"]})
                college_total += row["人數"]
            if current_college:
                export_list.append({"一級單位": current_college, "職稱": "小計", "人數": college_total})
