...
class TeacherStatsApp:
    ...
    def gen_d5_logic(self):
        ...
        try:
            # 讀取各系所教師的所有工作表並合併
            xl = pd.ExcelFile(self.file1_path)
            all_dfs = [pd.read_excel(self.file1_path, sheet_name=s) for s in xl.sheet_names]
            df = pd.concat(all_dfs, ignore_index=True)
            
            # 簡單清洗與格式化 (針對 D5 底稿)
            self.d5_df = df # 暫存供匯出使用
            
            self.d5_out.delete(1.0, tk.END)
            self.d5_out.insert(tk.END, f"【D5 報表計算完成】\n資料來源：{os.path.basename(self.file1_path)}\n共計：{len(df)} 筆異動記錄\n" + "-"*50 + "\n")
            self.d5_out.insert(tk.END, "預覽前 10 筆資料：\n")
            self.d5_out.insert(tk.END, df.head(10).to_string())
    ...
    def setup_tab5(self, f): self.create_report_ui(f, "給主計室", "生成每月主計報表。", "生成月報", lambda o: o.insert("1.0", "主計月報功能..."))
