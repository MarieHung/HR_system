...
class TeacherStatsApp:
    def __init__(self, root):
        self.root = root
        self.root.title("教師資料管理中心 - 行政綜合業務系統 (V7.3)")
    ...
    def setup_tab1(self, f):
        ...
        categories = ["新制職員", "助教", "技工工友合計", "教師", "約聘教師", "駐衛警察隊", "專案教學人員", "講座教授", "客座教授", "醫事人員"]
        for i, cat in enumerate(categories):
            var = tk.BooleanVar()
            self.tab1_cat_vars[cat] = var
            ttk.Checkbutton(cat_grid, text=cat, variable=var, command=self.refresh_tab1_titles).grid(row=i//5, column=i%5, sticky=tk.W, padx=10, pady=2)
    ...
    def setup_tab3(self, f):
        ...
        # 手動輸入區
        manual_frame = ttk.LabelFrame(main, text="手動輸入額外人員數據 (約用、兼任)", padding=10)
        manual_frame.pack(fill=tk.X, pady=10)
        
        # 第一排：約用人員
        ttk.Label(manual_frame, text="約用人員 - 總人數:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.E)
        self.manual_total_e = ttk.Entry(manual_frame, width=8)
        self.manual_total_e.grid(row=0, column=1, padx=5); self.manual_total_e.insert(0, "0")
        ttk.Label(manual_frame, text="男:").grid(row=0, column=2, padx=5); self.manual_m_e = ttk.Entry(manual_frame, width=8)
        self.manual_m_e.grid(row=0, column=3, padx=5); self.manual_m_e.insert(0, "0")
        ttk.Label(manual_frame, text="女:").grid(row=0, column=4, padx=5); self.manual_f_e = ttk.Entry(manual_frame, width=8)
        self.manual_f_e.grid(row=0, column=5, padx=5); self.manual_f_e.insert(0, "0")

        # 第二排：兼任教師 (新需求)
        ttk.Label(manual_frame, text="兼任教師 - 總人數:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.E)
        self.pt_total_e = ttk.Entry(manual_frame, width=8)
        self.pt_total_e.grid(row=1, column=1, padx=5); self.pt_total_e.insert(0, "0")
        ttk.Label(manual_frame, text="男:").grid(row=1, column=2, padx=5); self.pt_m_e = ttk.Entry(manual_frame, width=8)
        self.pt_m_e.grid(row=1, column=3, padx=5); self.pt_m_e.insert(0, "0")
        ttk.Label(manual_frame, text="女:").grid(row=1, column=4, padx=5); self.pt_f_e = ttk.Entry(manual_frame, width=8)
        self.pt_f_e.grid(row=1, column=5, padx=5); self.pt_f_e.insert(0, "0")

        ttk.Button(main, text="📊 生成全校人員統計報表 (含手動輸入項)", command=self.gen_vice_president_report).pack(pady=5)
    ...
    def gen_vice_president_report(self):
        ...
        # 讀取手動輸入
        try:
            m_total = int(self.manual_total_e.get())
            m_m = int(self.manual_m_e.get())
            m_f = int(self.manual_f_e.get())
            pt_total = int(self.pt_total_e.get())
            pt_m = int(self.pt_m_e.get())
            pt_f = int(self.pt_f_e.get())
        except ValueError:
            messagebox.showerror("錯誤", "手動輸入欄位請填入數字")
            return

        target_sheets = ["教師", "約聘教師", "專案教學人員", "講座教授", "客座人員", "研究人員", "新制職員", "駐衛警察隊", "技工工友合計"]
        try:
            xl = pd.ExcelFile(self.file1_path)
            self.vp_out.delete(1.0, tk.END)
            self.vp_out.insert(tk.END, f"【各類人員員額與性別統計摘要 - 副校長】\n日期：{datetime.now().strftime('%Y-%m-%d')}\n" + "="*70 + "\n")
            
            grand_total = m_total + pt_total
            grand_m = m_m + pt_m
            grand_f = m_f + pt_f
            ...
            # 顯示手動填報項
            self.vp_out.insert(tk.END, f"● 約用人員(填報) | {m_total:>8} | 男:{m_m:>4} | 女:{m_f:>4}\n")
            self.vp_out.insert(tk.END, f"● 兼任教師(填報) | {pt_total:>8} | 男:{pt_m:>4} | 女:{pt_f:>4}\n")
            
            self.vp_out.insert(tk.END, "-"*70 + f"\n【全校總計】人員總數：{grand_total} 名 (男：{grand_m} / 女：{grand_f})\n" + "="*70)
        except Exception as e:
    ...
    def setup_tab7(self, f):
        ...
        data_items = ["姓名", "服務單位", "一級單位", "身分證統一編號", "職稱", "性別", "薪點", "最高學位", "出生年月日", "到校日期", "EMI", "本校電子信箱", "校外電子信箱"]
        ...
        self.app_filename_e = ttk.Entry(export_f, width=30); self.app_filename_e.insert(0, "個資申請名單")
    ...
    def preview_app_data(self):
        ...
        if df is not None:
            self.app_out.delete(1.0, tk.END); self.app_out.insert(tk.END, f"人數：{len(df)}\n")
