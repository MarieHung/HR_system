...
class TeacherStatsApp:
    def __init__(self, root):
        self.root = root
        self.root.title("教師資料管理中心 - 行政綜合業務系統 (V7.1)")
    ...
    def calculate1_v7(self):
        ...
        if not sel:
            messagebox.showwarning("提示", "請勾選項目")
            return
        ...
        self.res_text1.insert(tk.END, f"【統計結果】\n{'職稱名稱':<25} | {'統計人數':>5}\n" + "-"*35 + "\n")
        total = 0
        for t in sel:
            c = cnt.get(t, 0)
    ...
    # --- 報表分頁 3-6 ---
    # --- 分頁 3: 給副校長 (V7.2 全校員額統計) ---
    def setup_tab3(self, f):
        main = ttk.Frame(f, padding="20")
        main.pack(fill=tk.BOTH, expand=True)
        ttk.Label(main, text="呈 副校長 - 全校各類人員員額統計週報", font=('Microsoft JhengHei', 16, 'bold'), foreground="#1a5276").pack(pady=(0, 10))
        
        ttk.Label(main, text="說明：系統將自動讀取教師、職員、技工等 9 個工作表並產出彙總人數。", foreground="blue").pack(pady=(0, 20))

        btn_f = ttk.Frame(main)
        btn_f.pack(fill=tk.X, pady=5)
        ttk.Button(btn_f, text="📊 一鍵生成全校人員統計報表", command=self.gen_vice_president_report).pack(side=tk.LEFT)
    ...
    def gen_vice_president_report(self):
        ...
        target_sheets = [
            "教師", "約聘教師", "專案教學人員", "講座教授", 
            "客座人員", "研究人員", "新制職員", "駐衛警察隊", "技工工友"
        ]
        
        try:
            xl = pd.ExcelFile(self.file1_path)
            self.vp_out.delete(1.0, tk.END)
            self.vp_out.insert(tk.END, f"【各類人員員額統計摘要表 - 呈 副校長】\n")
            self.vp_out.insert(tk.END, f"報表日期：{datetime.now().strftime('%Y-%m-%d')}\n")
            self.vp_out.insert(tk.END, "="*60 + "\n\n")
            ...
            for sheet in target_sheets:
                if sheet in xl.sheet_names:
                    ...
                    count = len(df)
                    self.vp_out.insert(tk.END, f"● {sheet:<15} ： {count:>5} 名\n")
                    
                    grand_total += count
                    if sheet in ["教師", "約聘教師", "專案教學人員", "講座教授", "客座人員", "研究人員"]:
                        teacher_total += count
                    else:
                        staff_total += count
                else:
                    self.vp_out.insert(tk.END, f"○ {sheet:<15} ： （找不到此工作表）\n")

            self.vp_out.insert(tk.END, "\n" + "-"*60 + "\n")
            self.vp_out.insert(tk.END, f"【全校總計彙整】\n")
            self.vp_out.insert(tk.END, f"   - 教學/研究人員小計 ： {teacher_total:>5} 名\n")
            self.vp_out.insert(tk.END, f"   - 行政/技術人員小計 ： {staff_total:>5} 名\n")
            self.vp_out.insert(tk.END, f"   - 全校人員總計      ： {grand_total:>5} 名\n")
            self.vp_out.insert(tk.END, "\n" + "="*60 + "\n")
            self.vp_out.insert(tk.END, "備註：本報表係由系統自動彙整『各系所教師.xlsx』資料產出。")

        except Exception as e:
            messagebox.showerror("報表生成失敗", str(e))
    ...
    def setup_tab7(self, f):
        ...
        self.file_label_app = ttk.Label(main, text="資料來源：各系所教師.xlsx", foreground="blue")
        self.file_label_app.pack()
        
        cat_f = ttk.LabelFrame(main, text="1. 請勾選人員類別 (工作表)", padding=10)
        cat_f.pack(fill=tk.X, pady=5)
        self.cat_vars = {}
        cat_grid = ttk.Frame(cat_f)
        cat_grid.pack(fill=tk.X, padx=5)
        ...
        item_f = ttk.LabelFrame(main, text="2. 請勾選資料項目 (欄位)", padding=10)
    ...
    def preview_app_data(self):
        ...
        if df is not None:
            ...
            self.app_out.insert(tk.END, f"人數：{len(df)}\n")
