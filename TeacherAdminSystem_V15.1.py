...
TEACHER_DATA_FILE = "各系所教師_跑程式用.xlsx"
ARRIVAL_LEAVE_FILE = "0910501新進及離職人員名單_跑程式用.xlsx"
CHANGE_DATA_FILE = "0950516總異動(完整版)_跑程式用.xlsx"
TEACHER_DATA_PATH = os.path.join(BASE_DIR, TEACHER_DATA_FILE)
ARRIVAL_LEAVE_PATH = os.path.join(BASE_DIR, ARRIVAL_LEAVE_FILE)
CHANGE_DATA_PATH = os.path.join(BASE_DIR, CHANGE_DATA_FILE)
TAB8_MAX_RESULTS = 1000
...
class TeacherStatsApp:
    def __init__(self, root):
        self.root = root
        self.root.title("教師資料管理中心 - 行政綜合業務系統 (V15.0)")
        ...
        # 建立 10 個主分頁
        tabs = [" 🔍 職稱統計 ", " 📅 到離查詢 ", " 📊 給副校長 ", " 📄 D5 報表 ", " 🛌 留職停薪 ", " 🌏 赴大陸統計 ", " 📝 個資申請單 ", " 📑 人事異動單 ", " 🏫 校區人數 ", " 🏢 學院職稱統計 "]
        self.tab_frames = []
        for text in tabs:
        ...
        self.setup_tab11(self.tab_frames[9]) # 新增：學院職稱統計
    ...
    def setup_tab1(self, f):
        ...
        self.file_label1 = ttk.Label(file_f, text=f"固定讀取：{TEACHER_DATA_FILE}", foreground="gray")
    ...
    def load_file1_v7(self):
        ...
        if not os.path.exists(p):
            self.file1_path = None
            self.file_label1.config(text=f"找不到檔案：{TEACHER_DATA_FILE}", foreground="red")
            return
        self.file1_path = p
        ...
        # 自動更新校區統計的工作表清單
        if hasattr(self, 'refresh_tab9_sheets') and hasattr(self, 'sheet_container9'):
    ...
    def calculate1_v7(self):
        ...
        # 定義特殊排序
        title_rank = {"助理教授": 1, "副教授": 2, "教授": 3}
        # 排序選擇的職稱
        sorted_sel = sorted(sel, key=lambda x: title_rank.get(x, 99))
    ...
    def setup_tab2(self, f):
        ...
        file_f.pack(fill=tk.X, pady=5)
        self.file_label2 = ttk.Label(file_f, text=f"固定讀取：{ARRIVAL_LEAVE_FILE}", foreground="gray", wraplength=300)
    ...
    def load_file2_v7(self):
        p = ARRIVAL_LEAVE_PATH
        if not os.path.exists(p):
            self.file2_path = None
            self.file_label2.config(text=f"找不到檔案：{ARRIVAL_LEAVE_FILE}", foreground="red")
            self.search_btn2.config(state=tk.DISABLED)
            return
        self.file2_path = p
    ...
    def gen_d5_logic(self):
        ...
        try:
            ...
            if "教師" in all_sheets:
                ...
                df_tech = df[is_tech_mask].copy(); df_normal = df[~is_tech_mask].copy()
                if not df_normal.empty:
                    df_normal['D5類別'] = "教師"; df_normal['來源工作表'] = "教師"; cat_data["教師"].append(df_normal)
                if not df_tech.empty:
                    df_tech['D5類別'] = "聘任"; df_tech['來源工作表'] = "教師(專業技術人員)"; cat_data["聘任"].append(df_tech)
            ...
            for s in ["研究人員"]:
                if s in all_sheets:
                    ...
                    if c_t:
                        df = df[df[c_t].astype(str).str.strip().replace(['nan','','None','nan '], pd.NA).notna()]
                    df['D5類別'] = "聘任"; df['來源工作表'] = s; cat_data["聘任"].append(df)
            ...
            if "助教" in all_sheets:
                ...
                if c_check:
                    ...
                    df_old = df[is_old_mask].copy(); df_old['D5類別'] = "教師"; df_old['來源工作表'] = "助教(舊制)"
                    if not df_old.empty: cat_data["教師"].append(df_old)
                    is_new_mask = df[c_check].astype(str).str.contains("新制助教")
                    df_new = df[is_new_mask].copy(); df_new['D5類別'] = "聘任"; df_new['來源工作表'] = "助教(新制)"
                    if not df_new.empty: cat_data["聘任"].append(df_new)
                else:
                    df['D5類別'] = "聘任"; df['來源工作表'] = "助教(未區分)"; cat_data["聘任"].append(df)

            final_list = []
            summary_text = "【D5 類別來源明細】\n"
            for name, dfs in cat_data.items():
                if dfs:
                    combined = pd.concat(dfs, ignore_index=True); final_list.append(combined)
            ...
            if not final_list: messagebox.showinfo("提示", "未找到相關工作表資料"); return
        ...
        except Exception as e: messagebox.showerror("計算失敗", str(e))

    def export_d5_excel(self):
        if not hasattr(self, 'd5_df'): messagebox.showwarning("提示", "請先點擊『開始計算D5報表』！"); return
    ...
    def setup_tab5(self, f):
        ...
        ttk.Label(main, text="說明：篩選且『留職停薪』欄位有資料的人員。", foreground="blue").pack(pady=(0, 15))
        file_name_f = ttk.Frame(main); file_name_f.pack(fill=tk.X, pady=5)
        ttk.Label(file_name_f, text="匯出檔案名稱:").pack(side=tk.LEFT, padx=5)
        self.unpaid_filename_e = ttk.Entry(file_name_f, width=30); self.unpaid_filename_e.insert(0, f"留職停薪名單_{datetime.now().strftime('%m%d')}"); self.unpaid_filename_e.pack(side=tk.LEFT, padx=5)
        btn_f = ttk.Frame(main); btn_f.pack(fill=tk.X, pady=10)
        ttk.Button(btn_f, text="🔍 開始提取留職停薪名單", command=self.gen_unpaid_leave_logic).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_f, text="📥 匯出 Excel", command=self.export_unpaid_leave_excel).pack(side=tk.LEFT, padx=5)
        self.unpaid_out = tk.Text(main, font=('Microsoft JhengHei', 11), bg="#fdf2f2"); self.unpaid_out.pack(fill=tk.BOTH, expand=True, pady=10)

    def gen_unpaid_leave_logic(self):
        if not self.file1_path: messagebox.showwarning("提示", "載入『各系所教師』檔案！"); return
        target_sheets = ["新制職員", "助教", "教師", "稀少性科技人員", "研究人員"]
        try:
            xl = pd.ExcelFile(self.file1_path); found_dfs = []; summary_info = "【留職停薪提取明細】\n"
            for sheet in target_sheets:
                if sheet in xl.sheet_names:
                    df = pd.read_excel(self.file1_path, sheet_name=sheet)
                    ...
                    if col:
                        ...
                        if not filtered_df.empty:
                            filtered_df['來源工作表'] = sheet; found_dfs.append(filtered_df)
                            summary_info += f"● {sheet:<12} | 找到 {len(filtered_df):>3} 筆\n"
                        else: summary_info += f"○ {sheet:<12} | 無符合紀錄\n"
                    else: summary_info += f"× {sheet:<12} | (找不到欄位)\n"
                else: summary_info += f"× {sheet:<12} | (找不到工作表)\n"
            if not found_dfs:
                self.unpaid_out.delete(1.0, tk.END); self.unpaid_out.insert(tk.END, summary_info + "\n未找到紀錄。"); return
        ...
        except Exception as e: messagebox.showerror("提取失敗", str(e))

    def export_unpaid_leave_excel(self):
        if not hasattr(self, 'unpaid_df'): messagebox.showwarning("提示", "先點擊提取！"); return
        out_name = self.unpaid_filename_e.get().strip()
        if not out_name: return
        try:
            save_path = f"{out_name}.xlsx"
            with pd.ExcelWriter(save_path) as writer: self.unpaid_df.to_excel(writer, sheet_name="留職停薪", index=False)
            messagebox.showinfo("成功", f"匯出至 {save_path}")
        except Exception as e: messagebox.showerror("匯出失敗", str(e))
    ...
    def load_manager_list(self):
        ...
        if not p: return
        try:
            xl = pd.ExcelFile(p); sheet_name = xl.sheet_names[0]
            df = pd.read_excel(p, sheet_name=sheet_name)
            self.file_label_t.config(text=os.path.basename(p), foreground="black")
            # 統計邏輯 (略，同 V14)
            messagebox.showinfo("成功", "主管統計完成")
        except Exception as e: messagebox.showerror("統計失敗", str(e))

    def process_teacher_managers(self):
        p = filedialog.askopenfilename(filetypes=[("Excel files", "*.xlsx;*.xls")])
        if not p: return
        try:
            xl = pd.ExcelFile(p); df = pd.read_excel(p, sheet_name=0)
            messagebox.showinfo("成功", "教兼主管處理完成")
        except Exception as e: messagebox.showerror("處理失敗", str(e))
    ...
    def setup_tab7(self, f):
        ...
        categories = ["新制職員", "助教", "技工工友合計", "教師", "約聘教師", "駐衛警察隊", "專案教學人員", "講座教授", "客座人員", "醫事人員", "研究人員", "稀少性科技人員"]
        for i, cat in enumerate(categories):
        ...
        data_items = ["姓名", "服務單位", "一級單位", "身分證統一編號", "職稱", "性別", "薪點", "最高學位", "出生年月日", "到校日期", "聘期起", "聘期迄", "年齡", "服務本校年資", "到校前本校年資", "EMI", "本校電子信箱", "校外電子信箱", "留職停薪", "帶職帶薪","國籍","雙重國籍"]
    ...
    def setup_tab8(self, f):
        ...
        self.tab8_file_path = CHANGE_DATA_PATH
        main = ttk.Frame(f, padding="10")
        main.pack(fill=tk.BOTH, expand=True)
        left_panel = ttk.Frame(main, width=350)
        left_panel.pack(side=tk.LEFT, fill=tk.BOTH, padx=(0, 10))
        ...
        file_f.pack(fill=tk.X, pady=5)
        self.tab8_file_label = ttk.Label(file_f, text=f"固定讀取：{CHANGE_DATA_FILE}", foreground="gray", wraplength=300)
        ...
        column_widths = {"姓名": 80, "原單位": 150, "原職稱": 100, "動態": 120, "新單位": 150, "新職稱": 100, "生效日期": 100, "備註": 150}
        for c in cols:
    ...
    def load_file_changes(self):
        if os.path.exists(self.tab8_file_path):
            self.tab8_file_label.config(text=os.path.basename(self.tab8_file_path), foreground="black")
            self.reload_file_changes()
        else:
            self.tab8_file_label.config(text=f"找不到檔案：{CHANGE_DATA_FILE}", foreground="red")

    def reload_file_changes(self):
        try:
            wb = load_workbook(self.tab8_file_path, read_only=True, data_only=True)
            for w in self.sheet_container8.winfo_children(): w.destroy()
            self.tab8_sheet_vars = {}
            for name in wb.sheetnames:
                var = tk.BooleanVar(value=True); self.tab8_sheet_vars[name] = var
                cb = ttk.Checkbutton(self.sheet_container8, text=name, variable=var); cb.pack(fill=tk.X)
            wb.close()
        except Exception as e:
    ...
    def search_changes_logic(self):
        ...
        try:
            self.tab8_search_btn.config(state=tk.DISABLED)
            ...
            if is_special_fmt:
                result_count, reached_limit = self.search_special_change_file(sel_sheets, d_val, n_val, k_val)
                if reached_limit:
                    messagebox.showinfo("提示", f"結果超過 {TAB8_MAX_RESULTS} 筆，已先顯示前 {TAB8_MAX_RESULTS} 筆。請輸入更精確的日期、姓名或關鍵字。")
                return
            ...
            for s in sel_sheets:
                ...
                res = df[mask]
                for _, r in res.iterrows():
                    if result_count >= TAB8_MAX_RESULTS:
                        reached_limit = True
                        break
                    def gv_gen(col): return str(r.get(col, "")).replace("nan", "-").strip() if col else "-"
                    vals = [gv_gen(c_name), gv_gen(c_orig_u), gv_gen(c_orig_t), gv_gen(c_status), gv_gen(c_new_u), gv_gen(c_new_t), gv_gen(c_date), gv_gen(c_rem)]
            ...
            self.tab8_search_btn.config(state=tk.NORMAL)

    def search_special_change_file(self, sel_sheets, d_val, n_val, k_val):
        wb = load_workbook(self.tab8_file_path, read_only=True, data_only=True)
        result_count = 0
        reached_limit = False
        try:
            for sheet_name in sel_sheets:
                if sheet_name not in wb.sheetnames:
                    continue
                ws = wb[sheet_name]
                preview_rows = list(ws.iter_rows(min_row=1, max_row=5, values_only=True))
                start_row = 5
                if len(preview_rows) > 3 and self.cell_has_value(preview_rows[3][0] if preview_rows[3] else None):
                    start_row = 4
                elif len(preview_rows) > 4 and self.cell_has_value(preview_rows[4][0] if preview_rows[4] else None):
                    start_row = 5
                for row in ws.iter_rows(min_row=start_row, values_only=True):
                    if result_count >= TAB8_MAX_RESULTS:
                        reached_limit = True
                        break
                    if not self.cell_has_value(self.row_value(row, 0)) and not self.cell_has_value(self.row_value(row, 3)):
                        continue
                    date_text = self.special_change_date_text(row)
                    name_text = self.display_cell(self.row_value(row, 0))
                    if n_val and n_val not in name_text:
                        continue
                    if d_val and d_val not in date_text:
                        continue
                    if k_val and not any(k_val in self.display_cell(value) for value in row if value is not None):
                        continue
                    vals = [
                        self.display_cell(self.row_value(row, 0)),
                        self.display_cell(self.row_value(row, 1)),
                        self.display_cell(self.row_value(row, 2)),
                        self.display_cell(self.row_value(row, 3)),
                        self.display_cell(self.row_value(row, 4)),
                        self.display_cell(self.row_value(row, 6)),
                        date_text,
                        self.display_cell(self.row_value(row, 11)),
                    ]
                    self.tree8.insert("", tk.END, values=vals)
                    result_count += 1
                if reached_limit:
    ...
    def special_change_date_text(self, row):
        parts = []
        for index in (7, 8, 9):
            value = self.row_value(row, index)
            if self.cell_has_value(value):
                parts.append(self.display_cell(value).replace("-", ""))
        ...
        if self.file1_path:
            try:
                ...
                for name in xl.sheet_names:
                    var = tk.BooleanVar(value=(name in default_targets)); self.tab9_sheet_vars[name] = var
                    ttk.Checkbutton(self.sheet_container9, text=name, variable=var).pack(fill=tk.X, padx=5, pady=1)
            except Exception as e:
    ...
    # --- 分頁 11: 學院職稱統計 (新增) ---
    def setup_tab11(self, f):
        main = ttk.Frame(f, padding="20")
        main.pack(fill=tk.BOTH, expand=True)
        ttk.Label(main, text="各學院(一級單位)教師人數統計", font=('Microsoft JhengHei', 14, 'bold')).pack(pady=5)
        
        src_f = ttk.LabelFrame(main, text="資料來源", padding=10)
        src_f.pack(fill=tk.X, pady=5)
        ttk.Label(src_f, text=f"固定讀取：{TEACHER_DATA_FILE}", foreground="gray").pack(anchor=tk.W)

        btn_f = ttk.Frame(main)
        btn_f.pack(fill=tk.X, pady=5)
        ttk.Button(btn_f, text="📊 執行統計 (依學院排序)", command=self.calculate_college_stats).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_f, text="📥 匯出 Excel", command=self.export_college_stats).pack(side=tk.LEFT, padx=5)

        table_f = ttk.Frame(main)
        table_f.pack(fill=tk.BOTH, expand=True, pady=5)
        cols = ("一級單位", "職稱", "人數", "比例")
        self.college_tree = ttk.Treeview(table_f, columns=cols, show="headings")
        vsb = ttk.Scrollbar(table_f, orient="vertical", command=self.college_tree.yview)
        self.college_tree.configure(yscrollcommand=vsb.set)
        
        widths = {"一級單位": 200, "職稱": 150, "人數": 100, "比例": 100}
        for col in cols:
            self.college_tree.heading(col, text=col)
            self.college_tree.column(col, width=widths.get(col, 100), anchor=tk.CENTER)
        
        self.college_tree.grid(row=0, column=0, sticky="nsew")
        vsb.grid(row=0, column=1, sticky="ns")
        table_f.grid_rowconfigure(0, weight=1)
        table_f.grid_columnconfigure(0, weight=1)

    def calculate_college_stats(self):
        for item in self.college_tree.get_children():
            self.college_tree.delete(item)

        if not os.path.exists(TEACHER_DATA_PATH):
            messagebox.showerror("錯誤", f"找不到檔案：{TEACHER_DATA_FILE}")
            return

        try:
            xl = pd.ExcelFile(TEACHER_DATA_PATH)
            # 預設統計「教師」工作表，若有「約聘教師」、「專案教學人員」也一併納入
            target_sheets = [s for s in ["教師", "約聘教師", "專案教學人員"] if s in xl.sheet_names]
            if not target_sheets:
                messagebox.showwarning("提示", "找不到 教師 相關工作表")
                return

            frames = [pd.read_excel(TEACHER_DATA_PATH, sheet_name=s) for s in target_sheets]
            df = pd.concat(frames, ignore_index=True)

            unit_col = next((c for c in df.columns if "一級單位" in str(c)), None)
            title_col = next((c for c in df.columns if any(k in str(c) for k in ["職稱", "職別"])), None)

            if not unit_col or not title_col:
                messagebox.showerror("錯誤", "找不到『一級單位』或『職稱』欄位")
                return

            # 清洗資料
            df = df[[unit_col, title_col]].copy()
            df.columns = ["一級單位", "職稱"]
            df["一級單位"] = df["一級單位"].astype(str).str.strip()
            df["職稱"] = df["職稱"].astype(str).str.strip()
            df = df[df["一級單位"].replace(["nan", "None", ""], pd.NA).notna()]
            df = df[df["職稱"].replace(["nan", "None", ""], pd.NA).notna()]
            title_order = ["助理教授", "副教授", "教授"]
            df = df[df["職稱"].isin(title_order)]

            if df.empty:
                messagebox.showwarning("提示", "找不到助理教授、副教授、教授資料")
                return

            # 統計
            stats = df.groupby(["一級單位", "職稱"]).size().reset_index(name="人數")
            
            # 計算各學院總人數
            college_total_map = stats.groupby("一級單位")["人數"].sum().to_dict()
            stats["比例"] = stats.apply(
                lambda row: f"{row['人數'] / college_total_map[row['一級單位']] * 100:.1f}%",
                axis=1,
            )

            # 定義職稱排序權重
            rank_map = {title: idx for idx, title in enumerate(title_order)}
            stats["rank_weight"] = stats["職稱"].apply(lambda x: rank_map.get(x, 99))

            # 排序：一級單位 -> 職稱權重
            stats = stats.sort_values(by=["一級單位", "rank_weight"])

            self.last_college_stats = stats.copy() # 保存以便匯出
            self.last_college_totals = college_total_map

            current_college = None
            rows_to_insert = []
            for _, row in stats.iterrows():
                if current_college and current_college != row["一級單位"]:
                    rows_to_insert.append((current_college, "--- 小計 ---", college_total_map[current_college], "100.0%"))
                current_college = row["一級單位"]
                rows_to_insert.append((row["一級單位"], row["職稱"], row["人數"], row["比例"]))
            
            if current_college:
                rows_to_insert.append((current_college, "--- 小計 ---", college_total_map[current_college], "100.0%"))
            rows_to_insert.append(("", "", "", ""))
            rows_to_insert.append(("各學院總人數統計", "", "", ""))
            for college, total in sorted(college_total_map.items()):
                rows_to_insert.append((college, "總人數", total, "100.0%"))

            for r in rows_to_insert:
                tag = 'total' if "小計" in r[1] or r[1] == "總人數" else ''
                if r[0] == "各學院總人數統計":
                    tag = 'section'
                self.college_tree.insert("", tk.END, values=r, tags=(tag,))

            self.college_tree.tag_configure('total', background='#e8f4f8', font=('Microsoft JhengHei', 10, 'bold'))
            self.college_tree.tag_configure('section', background='#d9ead3', font=('Microsoft JhengHei', 10, 'bold'))

        except Exception as e:
            messagebox.showerror("統計失敗", str(e))

    def export_college_stats(self):
        if not hasattr(self, 'last_college_stats') or self.last_college_stats is None:
            messagebox.showwarning("提示", "請先執行統計！")
            return
        
        try:
            save_p = f"學院教師職稱統計_{datetime.now().strftime('%m%d')}.xlsx"
            export_list = []
            current_college = None
            college_total_map = getattr(self, 'last_college_totals', {})
            for _, row in self.last_college_stats.iterrows():
                if current_college and current_college != row["一級單位"]:
                    export_list.append({"一級單位": current_college, "職稱": "小計", "人數": college_total_map.get(current_college, ""), "比例": "100.0%"})
                current_college = row["一級單位"]
                export_list.append({"一級單位": row["一級單位"], "職稱": row["職稱"], "人數": row["人數"], "比例": row["比例"]})
            if current_college:
                export_list.append({"一級單位": current_college, "職稱": "小計", "人數": college_total_map.get(current_college, ""), "比例": "100.0%"})
            export_list.append({"一級單位": "", "職稱": "", "人數": "", "比例": ""})
            export_list.append({"一級單位": "各學院總人數統計", "職稱": "", "人數": "", "比例": ""})
            for college, total in sorted(college_total_map.items()):
                export_list.append({"一級單位": college, "職稱": "總人數", "人數": total, "比例": "100.0%"})
            
            pd.DataFrame(export_list).to_excel(save_p, index=False)
            messagebox.showinfo("成功", f"統計報表已匯出至：{save_p}")
        except Exception as e:
