...
class TeacherStatsApp:
    def __init__(self, root):
        ...
        # 建立 8 個主分頁
        tabs = [" 🔍 職稱統計 ", " 📅 到離查詢 ", " 📊 給副校長 ", " 📄 D5 報表 ", " 🛌 留職停薪 ", " 🌏 赴大陸統計 ", " 📝 個資申請單 ", " 📑 人事異動單 "]
        self.tab_frames = []
        for text in tabs:
    ...
    def setup_tab8(self, f):
        ...
        for c in cols:
            ...
            self.tree8.column(c, width=100, anchor=tk.CENTER)
    ...
    def search_changes_logic(self):
        ...
        try:
            for s in sel_sheets:
                ...
                c_unit = next((c for c in cols if "單位" in c), None)
                c_title = next((c for c in cols if any(k in c for k in ["職稱", "職別"])), None)
                c_status = next((c for c in cols if any(k in c for k in ["動態", "異動"])), None)
                ...
                for _, r in res.iterrows():
                    vals = [
                        r.get(c_name, "") if c_name else "",
                        r.get(c_unit, "") if c_unit else "",
                        r.get(c_title, "") if c_title else "",
                        r.get(c_status, "") if c_status else "",
                        r.get(c_date, "") if c_date else "",
                        r.get(c_rem, "") if c_rem else ""
                    ]
                    self.tree8.insert("", tk.END, values=vals)
                    
        except Exception as e:
            messagebox.showerror("搜尋出錯", str(e))
